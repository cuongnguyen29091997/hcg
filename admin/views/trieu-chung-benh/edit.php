<?php 
  $tcb['conditions'] = explode(";", $tcb['conditions']);
  ob_start();
...
<!-- Content -->
  <?php 
      require_once('admin/views/layouts/message.php');
   ?>
  <form method="POST" action="/cms-admin/trieu-chung-benh/update/<?php echo $tcb['id'] ?>">
    ...
    <div class="form-group">
      ...
      <select name="result" class="form-control" id="f_result">
        <?php foreach ($sickness as $sick) { 
          $checked = ($sick->id == $tcb['result'] ? "selected" : "");
        ?>
          <option value="<?php echo $sick->id ?>" <?php echo $checked ?>><?php echo $sick->name; ?></option>";
        <?php } ?>
      </select>
    </div>
